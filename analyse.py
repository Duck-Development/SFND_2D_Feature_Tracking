...
def main():
  ...
  with open(filepath) as fp:
    for line in fp:
      ...
      if line.startswith('Start DecType'):
        lineA = line.split('\n')
        lineA = lineA[0].split(' ')
        detector = lineA[3]
        descriptor = lineA[5]
      ...
      if line.find('detection with n=') > 0: 
        lineA = line.split('detection with n=')
        
        lineA = lineA[1].split(' ')
        keyPoint = int(lineA[0])
        time = float(lineA[-2])
        ...
        frames[frame][detector][descriptor]['keyPointCnt'] = keyPoint
        frames[frame][detector][descriptor]['keyPointTime'] = time

      if line.find('matched keypoint') >= 0: 
        lineA = line.split(' ')
        match = int(lineA[-1])
        ...
        if not (descriptor in frames[frame][detector]):
          frames[frame][detector][descriptor] = {}
        frames[frame][detector][descriptor]['match'] = match
        #print(match)
      if line.find('descriptor extraction in') > 0:
        lineA = line.split(' ')
        time = float(lineA[-2])
        if not (detector in frames[frame]):
          frames[frame][detector] = {}
        if not (descriptor in frames[frame][detector]):
          frames[frame][detector][descriptor] = {}
        frames[frame][detector][descriptor]['descriporTime'] = time
